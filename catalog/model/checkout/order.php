	...
	public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false, $override = false) {
		...
		if ($order_info) {
			...
			if (!$order_info['order_status_id'] && $order_status_id) {
				...
				$data['invoice_download'] = $order_info['store_url'] . 'index.php?route=account/order/info&order_id=' . $order_id . '&action=download';
				...
				$message = html_entity_decode($this->config->get('config_new_order_msg'), ENT_QUOTES, 'UTF-8');;
				$replace = array(
						'{STORENAME}'   => $data['store_name'],
						'{TELEPHONE}'   => $data['telephone'],
						'{EMAIL}'       => $data['email'],
						'{IP}'          => $data['ip'],
						'{ORDERDETAIL}' => $this->load->view('mail/order', $data),
						'{ORDERLINK}'   => $data['link'],
						...
						'{INVOICELINK}' => $data['invoice_download'],
					);
				if(empty($message)) {
					$message = implode("\n", array_keys($replace));
				}
				...
				if (in_array('order', (array)$this->config->get('config_mail_alert'))) {
					...
					$message = html_entity_decode($this->config->get('config_new_order_msg_admin'), ENT_QUOTES, 'UTF-8');;
					$replace = array(
							'{STORENAME}'   => $data['store_name'],
							'{TELEPHONE}'   => $data['telephone'],
							'{EMAIL}'       => $data['email'],
			...
			if ($order_info['order_status_id'] && $order_status_id && $notify) {
				...
				$message .= $language->get('text_update_footer');
					...
					$message = html_entity_decode($this->config->get('config_update_order_msg'), ENT_QUOTES, 'UTF-8');;
					$replace = array(
							'{STORENAME}'   => $data['store_name'],
							'{TELEPHONE}'   => $data['telephone'],
							'{EMAIL}'       => $data['email'],
