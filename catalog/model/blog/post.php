...
class ModelBlogPost extends Model {
	...
	public function getPosts($data = array()) {
		...
		if (!empty($data['filter_popular'])) {
			$sql .= " AND p.popular = '" . (int)$data['filter_popular'] . "'";
		}

		if (!empty($data['filter_show_in_menu'])) {
			$sql .= " AND p.show_in_menu = '" . (int)$data['filter_show_in_menu'] . "'";
		}
	...
	public function getPostCategories($data) {
		$category_data = array();
		$category_data = $this->cache->get('post.categories.' . (int)$this->config->get('config_language_id') . '.' . (int)$this->config->get('config_store_id')  . '.' . (int)$data['limit']);
		...
		if (!$category_data) {
			...
			foreach ($query->rows as $result) {
				...
				if(count($category_data) >= $data['limit']){
					break;
				}
			...
			$this->cache->set('post.categories.' . (int)$this->config->get('config_language_id') . '.' . (int)$this->config->get('config_store_id') . '.' . (int)$data['limit'], $tag_data);
		}

		return $category_data;
	}

	public function getNextPost($blog_post_id) {
		$query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "blog_post i LEFT JOIN " . DB_PREFIX . "blog_post_description id ON (i.blog_post_id = id.blog_post_id) LEFT JOIN " . DB_PREFIX . "blog_post_to_store i2s ON (i.blog_post_id = i2s.blog_post_id) WHERE i.blog_post_id > '" . (int)$blog_post_id . "' AND id.language_id = '" . (int)$this->config->get('config_language_id') . "' AND i2s.store_id = '" . (int)$this->config->get('config_store_id') . "' AND i.status = '1'");

		return $query->row;
	}

	public function getPrevPost($blog_post_id) {
		$query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "blog_post i LEFT JOIN " . DB_PREFIX . "blog_post_description id ON (i.blog_post_id = id.blog_post_id) LEFT JOIN " . DB_PREFIX . "blog_post_to_store i2s ON (i.blog_post_id = i2s.blog_post_id) WHERE i.blog_post_id < '" . (int)$blog_post_id . "' AND id.language_id = '" . (int)$this->config->get('config_language_id') . "' AND i2s.store_id = '" . (int)$this->config->get('config_store_id') . "' AND i.status = '1'");
	...
	public function getLastPost() {
		$query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "blog_post i LEFT JOIN " . DB_PREFIX . "blog_post_description id ON (i.blog_post_id = id.blog_post_id) LEFT JOIN " . DB_PREFIX . "blog_post_to_store i2s ON (i.blog_post_id = i2s.blog_post_id) WHERE i.blog_post_id = (SELECT MAX(blog_post_id) FROM " . DB_PREFIX . "blog_post WHERE status = '1') AND id.language_id = '" . (int)$this->config->get('config_language_id') . "' AND i2s.store_id = '" . (int)$this->config->get('config_store_id') . "' AND i.status = '1'");

		return $query->row;
	}

	public function addLike($blog_post_id) {
		$query = $this->db->query("SELECT likes FROM " . DB_PREFIX . "blog_post WHERE blog_post_id = '" . (int)$blog_post_id . "'");

		$likes = (int) $query->row['likes'];
		$likes = $likes + 1;

		$this->db->query("UPDATE " . DB_PREFIX . "blog_post SET likes = '" . $likes . "' WHERE blog_post_id = '" . (int)$blog_post_id . "'");

		return $likes;
	}
	
	public function addDislike($blog_post_id) {
		$query = $this->db->query("SELECT dislikes FROM " . DB_PREFIX . "blog_post WHERE blog_post_id = '" . (int)$blog_post_id . "'");

		$dislikes = (int) $query->row['dislikes'];
		$dislikes = $dislikes + 1;

		$this->db->query("UPDATE " . DB_PREFIX . "blog_post SET dislikes = '" . $dislikes . "' WHERE blog_post_id = '" . (int)$blog_post_id . "'");
