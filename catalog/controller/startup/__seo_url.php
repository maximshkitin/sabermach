...
class ControllerStartupSeoUrl extends Controller {
	...
	public function index() {
		...
		if (!isset($this->request->get['_route_'])) {
            ...
            if (strpos($this->request->get['_route_'], 'blog/') !== false) {
	...
	public function rewrite($link) {
		...
		switch ($route) {
			...
			case 'blog/post':
                ...
                if ($query->num_rows && $query->row['keyword']) {
                    $url .= ((strpos($url,'/knowledge-base/') === false)?'/knowledge-base/':'/') . $query->row['keyword'];
                    unset($data[$key]);
                    return $url;
                }
                break;*/
		...
		if(count($rows) == count($queries)) {
			...
			foreach($queries as $query) {
				$seo_url .= '/' . str_replace('%2F','/', rawurlencode($aliases[$query]));
			}
		}
	...
	private function getPathByProduct($product_id) {
		...
		if (!isset($path[$product_id])) {
			$query = $this->db->query("SELECT category_id FROM " . DB_PREFIX . "product_to_category WHERE product_id = '" . $product_id . "' ORDER BY category_id DESC LIMIT 1");
	...
	private function getPathByCategory($category_id) {
		...
		if (!isset($path[$category_id])) {
			...
			$path[$category_id] = ($query->num_rows > 0) ? $query->row['path'] : false;
