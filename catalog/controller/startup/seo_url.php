...
class ControllerStartupSeoUrl extends Controller {
	public function index() {
		...
		if (isset($this->request->get['_route_'])) {
			...
			if (strpos($this->request->get['_route_'], 'knowledge-base/') !== false) {
                foreach ($parts as $part) {
                    ...
                    if ($query->num_rows) {
                        ...
                        if($url[0] != 'blog_category_id' && $url[0] != 'blog_post_id') {
                        	$this->request->get['route'] = 'blog/category';
                        }
                    }
                }
	...
	public function rewrite($link) {
		...
		foreach ($data as $key => $value) {
			if (isset($data['route'])) {
				if (($data['route'] == 'product/product' && $key == 'product_id') || (($data['route'] == 'product/manufacturer/info' || $data['route'] == 'product/product') && $key == 'manufacturer_id') || ($data['route'] == 'information/information' && $key == 'information_id')) {
	            ...
	            } elseif (($data['route'] == 'blog/category') || ($data['route'] == 'blog/post')) {
	                ...
	                if ($query->num_rows && $query->row['keyword']) {
	                	...
	                	$url .= ((strpos($url,'/knowledge-base/') === false)?'/knowledge-base/':'/');
