	...
	public function index($setting) {
		...
		if(!empty($setting['category_status'])) {
			...
			foreach ($categories as $category) {
				...
				$filter_data_items = array(
					'filter_blog_category_id'  => $category['blog_category_id'],
					'filter_show_in_menu' => 1
				);

				$posts = array();

				$items = $this->model_blog_post->getPosts($filter_data_items);
...
*/
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . 'extension/module/blog.tpl')) {
			return $this->load->view($this->config->get('config_template') . 'extension/module/blog.tpl', $data);
		} else {
			return $this->load->view('extension/module/blog.tpl', $data);
		}
	}
}