	...
	public function index() {
		...
		if (!empty($this->request->get['token'])) {
			...
			if ($customer_info && $this->customer->login($customer_info['email'], '', true)) {
				...
				$this->session->data['payment_address'] = $this->model_account_address->getAddress($this->customer->getAddressId(), 0);
				//}

				//if ($this->config->get('config_tax_customer') == 'shipping') {
				//if($customer_info['address_id'] == 0) {
				$this->session->data['shipping_address'] = $this->model_account_address->getAddress($this->customer->getAddressId(), 1);
				//} else {
				//	$this->session->data['shipping_address'] = $this->model_account_address->getAddress($this->customer->getAddressId(), 0);
				//}
				//}
				...
				if (empty($customer_info['avatar'])) {
					$this->session->data['guest']['avatar'] = $this->model_tool_image->resize('catalog/default-user-avatar.png', 100, 100);
				} else {
					$this->session->data['guest']['avatar'] = $this->model_tool_image->resize($customer_info['avatar'], 100, 100);
				}
		...
		$data['text_don_have_accaunt']        = $this->language->get('text_don_have_accaunt');
