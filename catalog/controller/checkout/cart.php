...
class ControllerCheckoutCart extends Controller {
	public function index() {
		if (!$this->customer->isLogged() && $this->cart->hasProducts() > 0) {
			$this->cart->clear();
			$this->response->redirect('/index.php?route=product/category&path=63');
		}
		...
		if ($this->config->get('config_checkout_id')) {
			...
			if ($information_info) {
				$data['text_agree'] = $this->url->link('information/information', 'information_id='.$this->config->get('config_checkout_id'), true);
			} else {
				$data['text_agree'] = '';
			}
		} else {
		...
		if ($this->cart->hasProducts() || !empty($this->session->data['vouchers'])) {
			...
			foreach ($products as $product) {
				...
				foreach ($product['option'] as $option) {
					...
					if ((int) $option['option_id'] == 13 || (int) $option['option_id'] == 14) {
						if ((int) $option['option_id'] == 13) {
							$hilt_data = array(
								'name'  => $option['name'],
								'value' => (utf8_strlen($value) > 20?utf8_substr($value, 0, 20).'..':$value)
							);
						}
						if ((int) $option['option_id'] == 14) {
							$color_data[] = array(
								'name'  => $option['name'],
								'value' => (utf8_strlen($value) > 20?utf8_substr($value, 0, 20).'..':$value)
							);
				...
				$data['products'][] = array(
					...
					'price'     => $price,
					'total'     => $total,
					'href'      => $this->url->link('product/product', 'path=63&product_id='.$product['product_id'])
				);
			}
			...
			foreach ($totals as $total) {
				if ($total['code'] == 'total') {
					$data['total'] = array(
						'title'    => $total['title'],
						'text'     => str_replace($replace, '', $this->currency->format($total['value'], $this->session->data['currency'])),
