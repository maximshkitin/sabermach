...
class ControllerCatalogProduct extends Controller {
	...
	protected function getForm() {
		...
		$data['text_remove_row']   = $this->language->get('text_remove_row');
		...
		$data['entry_additional_hilt']                     = $this->language->get('entry_additional_hilt');
		...
		$data['entry_technical_specification_description'] = $this->language->get('entry_technical_specification_description');
		$data['entry_included_description']                = $this->language->get('entry_included_description');
		$data['entry_included_image']                      = $this->language->get('entry_included_image');
		$data['entry_item_title']                          = $this->language->get('entry_item_title');
		$data['entry_included_items']                      = $this->language->get('entry_included_items');
		...
		if (!empty($data['languages']) && !empty($data['product_description'])) {
			foreach ($data['languages'] as $language) {
				if (!empty($data['product_description'][$language['language_id']]['included_image']) && is_file(DIR_IMAGE.$data['product_description'][$language['language_id']]['included_image'])) {
					$data['product_description'][$language['language_id']]['thumb'] = $this->model_tool_image->resize($data['product_description'][$language['language_id']]['included_image'], 100, 100);
				} else {
					$data['product_description'][$language['language_id']]['thumb'] = $this->model_tool_image->resize('no_image.png', 100, 100);
				}
				if (!empty($data['product_description'][$language['language_id']]['features_items'])) {
					foreach ($data['product_description'][$language['language_id']]['features_items'] as $key => $item) {
						if (!empty($item['image']) && is_file(DIR_IMAGE.$item['image'])) {
							$data['product_description'][$language['language_id']]['features_items'][$key]['thumb'] = $this->model_tool_image->resize($item['image'], 100, 100);
						} else {
							$data['product_description'][$language['language_id']]['features_items'][$key]['thumb'] = $this->model_tool_image->resize('no_image.png', 100, 100);
						}
					}
				}
			}
		...
		// hilt options
		$data['hilt_options'] = array();
		foreach ($data['product_options'] as $product_option) {
			if ($product_option['option_id'] == 13) {
				$data['hilt_options'] = $this->model_catalog_option->getOptionValues($product_option['option_id']);
			}
		}
		...
		foreach ($product_images as $product_image) {
			...
			$data['product_images'][] = array(
				...
				'hilt_option_id' => $product_image['hilt_option_id'],
