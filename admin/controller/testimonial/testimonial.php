...
class ControllerTestimonialTestimonial extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if (isset($this->request->get['filter_groups'])) {
				$url .= '&filter_groups=' . urlencode(html_entity_decode($this->request->get['filter_groups'], ENT_QUOTES, 'UTF-8'));
			}
	...
	protected function getList() {
		...
		foreach ($results as $result) {
			$data['reviews'][] = array(
				...
				'reply'      => ((int) $result['customer_id'] > 0) ? $this->url->link($this->modulePath . '/add', 'customer='.$result['customer_id'].'&token=' . $this->session->data['token'] . $url, 'SSL') : false
			);
		}
		...
		$data['sort_rgroups'] = $this->url->link($this->modulePath, 'token=' . $this->session->data['token'] . '&sort=groups' . $url, 'SSL');
	...
	protected function getForm() {
		...
		if (isset($this->error['groups'])) {
			$data['error_groups'] = $this->error['groups'];
		} else {
			$data['error_groups'] = '';
		}
		...
		if (isset($this->request->get['filter_groups'])) {
			$url .= '&filter_groups=' . urlencode(html_entity_decode($this->request->get['filter_author'], ENT_QUOTES, 'UTF-8'));
		}
		...
		if (isset($this->request->get['customer'])) {
            $data['customer'] = (int) $this->request->get['customer'];
        }
		...
		if (isset($this->request->post['groups'])) {
			$data['groups'] = $this->request->post['groups'];
		} elseif (!empty($review_info)) {
			$data['groups'] = $review_info['groups'];
		} else {
			$data['groups'] = '';
		}
		...
		if (isset($this->request->post['avatar'])) {
			...
			$data['avatar'] = $this->model_tool_image->resize($review_info['avatar'], 100, 100);
		} else {
			$data['avatar'] = '';
		}
		...
		if(isset($this->request->post['files'])) {
			foreach ($this->request->post['files'] as $file) {
				if(!empty($file) && is_file(DIR_IMAGE . $file)) {
					$image = array();
					$image['image'] = $file;
					$image['thumb'] = $this->model_tool_image->resize($file, 100, 100);
					$data['files'][] = $image;
				}
			}
		} elseif (!empty($review_info)) {
			$files = json_decode($review_info['files'], true);
		...
		if(!empty($review_info) && isset($review_info['customer_id']) && !empty($review_info['customer_id'])) {
			$this->load->model('customer/customer');
			$customer_info = $this->model_customer_customer->getCustomer($this->customer->getId());
			if(empty($customer_info['avatar'])) {
				$data['placeholder_avatar'] = $this->model_tool_image->resize('catalog/default-user-avatar.png', 100, 100);
			} else {
				$data['placeholder_avatar'] = $this->model_tool_image->resize($customer_info['avatar'], 100, 100);
			}
		} else {
			$data['placeholder_avatar'] = $this->model_tool_image->resize('catalog/default-user-avatar.png', 100, 100);
		}
