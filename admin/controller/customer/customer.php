...
class ControllerCustomerCustomer extends Controller {
	...
	public function import() {     
        ...
        $data['back'] = $this->url->link('customer/customer', 'token=' . $this->session->data['token'], 'SSL');
                ...
                if ((isset($this->request->files['file'])) && (is_uploaded_file($this->request->files['file']['tmp_name']))) {
                    ...
                    if($columns == 9)
                    {
                        for ($i = 0; $i < $k; $i++) {  // Excel row loop start
                            ...
                            $store_id = 0;
                            $customer_firstname = $this->getCell($xldata, $i, 1);
                            $customer_lastname = $this->getCell($xldata, $i, 2);

                            $email = $this->getCell($xldata, $i, 3);
                            $telephone = $this->getCell($xldata, $i, 4);
                            $fax = '';							
                            $password = $this->getCell($xldata, $i, 5);
                            $newsletter = 0;
							$ip = '127.0.0.1';
							...
							$company_id = '';
							$tax_id = '';
							$address_1 = $this->getCell($xldata, $i, 8);
							$address_2 = '';
							$city = '';
							$postcode = $this->getCell($xldata, $i, 9);
							$country = $this->getCell($xldata, $i, 6);
							$zone = $this->getCell($xldata, $i, 7);			
							
							$country_id=$this->model_customer_customer->getCountryId($country);

							if(!empty($country_id)) {
								$zone_id=$this->model_customer_customer->getZoneId($zone, $country_id);
							} else {
								$country_id='';
								$zone_id='';
							}			
								...
								$customer_array[$temp]['safe'] = 0;								
								
								$customer_array_add[$temp_add]['firstname'] = $customer_firstname;
								$customer_array_add[$temp_add]['lastname'] = $customer_lastname;
								...
								if($this->model_customer_customer->validEmail($email)) {
									$customer = $customer_array[$temp];
									$customer['address'] = array();
									$customer_array_add[$temp_add]['default'] = 1;
									$customer['address'][] = $customer_array_add[$temp_add];
									unset($customer_array_add[$temp_add]['default']);
									$customer['address'][] = $customer_array_add[$temp_add];
									if($this->model_customer_customer->addCustomer($customer)) {
										$customer_array[$temp]['import_status'] = 1;
									} else {
										$customer_array[$temp]['import_status'] = 'Error saving customer';
									}
								} else {
									$customer_array[$temp]['import_status'] = 'E-mail alredy exist';
								}
    ...
    public function customersampleexport()
    {
       ...
       //customer field
        $objPHPExcel->getActiveSheet()->SetCellValue('A'.$rowCount, 'Customer FirstName');
        $objPHPExcel->getActiveSheet()->SetCellValue('B'.$rowCount, 'Customer LastName');
        $objPHPExcel->getActiveSheet()->SetCellValue('C'.$rowCount, 'Email');
        $objPHPExcel->getActiveSheet()->SetCellValue('D'.$rowCount, 'Telephone');
        $objPHPExcel->getActiveSheet()->SetCellValue('E'.$rowCount, 'Password');

        //address field
        $objPHPExcel->getActiveSheet()->SetCellValue('G'.$rowCount, 'Country');
		$objPHPExcel->getActiveSheet()->SetCellValue('H'.$rowCount, 'Zone');
		$objPHPExcel->getActiveSheet()->SetCellValue('I'.$rowCount, 'Address 1');
		$objPHPExcel->getActiveSheet()->SetCellValue('J'.$rowCount, 'Postcode');
