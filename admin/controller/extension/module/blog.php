...
class ControllerExtensionModuleBlog extends Controller {
	...
	public function index() {
		...
		if (!isset($this->request->get['module_id'])) {
			$data['action'] = $this->url->link('extension/module/blog', 'token=' . $this->session->data['token'], 'SSL');
		} else {
			$data['action'] = $this->url->link('extension/module/blog', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], 'SSL');
		}
...
*/
		...
		$this->response->setOutput($this->load->view('extension/module/blog.tpl', $data));
	}
