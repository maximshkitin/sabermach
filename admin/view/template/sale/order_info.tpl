          ...
          <thead>
            <tr>
              ...
              <td style="width: 50%;" class="text-left">
                <?php echo $text_shipping_address; ?></td>
              <?php } ?>
            </tr>
          </thead>
          <tbody>
            <tr>
              ...
              <td class="text-left">
                ...
                <form id="tracing-code" class="form-vertical">
                  <div class="form-group">
                  <div class="col-sm-12">
                    <label class="control-label" for="input-tcode">Tracking code</label>
                    <div class="input-group">
                      <input type="text" class="form-control" placeholder="Tracking code..." name="tracking_code" value="<?php echo $tracking_code;?>" id="input-tcode">
...
$('#tracing-code').on('submit', function(e) {
  e.preventDefault();
  var link = 'index.php?route=sale/order/tcode&token=<?php echo $token; ?>&order_id=<?php echo $order_id; ?>';
  ...
  $.ajax({
    ...
    data: data,
    success: function(json) {
       $('#tracing-code .col-sm-12 .text-danger').remove();
      if(json.success) {
        if(json.message) {
          alert(json.message);
        }
        $('#tracing-code input[name="tracking_code"]').val(json.success);
      } else {
        $('#tracing-code .col-sm-12').append('<div class="text-danger">' + json.error + '</div>');
      }
    }
  });
})
