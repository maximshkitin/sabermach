...
class ModelCustomerCustomer extends Model {
	public function addCustomer($data) {
		...
		if (isset($data['address'])) {
			foreach ($data['address'] as $key => $address) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "address SET customer_id = '" . (int)$customer_id . "', firstname = '" . $this->db->escape($address['firstname']) . "', lastname = '" . $this->db->escape($address['lastname']) . "', company = '" . $this->db->escape($address['company']) . "', address_1 = '" . $this->db->escape($address['address_1']) . "', address_2 = '" . $this->db->escape($address['address_2']) . "', city = '" . $this->db->escape($address['city']) . "', postcode = '" . $this->db->escape($address['postcode']) . "', country_id = '" . (int)$address['country_id'] . "', zone_id = '" . (int)$address['zone_id'] . "', custom_field = '" . $this->db->escape(isset($address['custom_field']) ? json_encode($address['custom_field']) : '') . "', type = '".$key."'");
	...
	public function getZoneId($zone, $country_id) {
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "zone` WHERE `country_id` = '" . (int) $country_id ."' AND  `name` = '" . $this->db->escape($zone) . "'");

		return isset($query->row['zone_id']) ? $query->row['zone_id'] : '';
	}

	public function getCountryId($country) {
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "country` WHERE `name` = '" . $this->db->escape($country) . "'");

		return isset($query->row['country_id']) ? $query->row['country_id'] : '';
	}
